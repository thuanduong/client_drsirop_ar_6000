...
public class UISliderDot : MonoBehaviour
{
    #region Variables
    ...
    [SerializeField] private bool _useGameObject;
    ...
    public virtual void ChangeActiveState(bool active)
    {
        ...
        if (_useGameObject != default)
        {
            if (_On != default) _On.SetActive(active);
            if (_Off != default) _Off.SetActive(!active);
        }
